...
import { Shield, Eye, EyeOff, MapPin, Lock, Phone } from 'lucide-react'
...
const bgPattern = {
  backgroundImage: `linear-gradient(45deg, #610c27 25%, transparent 25%), linear-gradient(-45deg, #610c27 25%, transparent 25%), linear-gradient(45deg, transparent 75%, #610c27 75%), linear-gradient(-45deg, transparent 75%, #610c27 75%)`,
  ...
  return (
    <div className="min-h-screen flex bg-[#0a0a0a] relative overflow-hidden">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        ...
        />
        {/* Soft gradient overlay for card area legibility */}
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="w-full max-w-[420px]">
          ...
          >
            ...
            <div className="relative z-10">
              <div className="mb-7">
                <h1 className="text-2xl sm:text-[28px] font-bold font-display text-[#1a1a1a] italic uppercase tracking-tight mb-1.5">Welcome Back</h1>
                <p className="text-sm text-[#666] font-medium">Sign in to continue to your account</p>
              </div>

              <form onSubmit={handleSubmit} className="space-y-4">
                <input id="login-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Enter Email / Phone No" className="w-full bg-[#f9f8f6]/50 border border-[#e0ddd8] rounded-xl px-5 py-3.5 text-[#1a1a1a] placeholder-[#999] text-sm focus:outline-none focus:ring-2 focus:ring-[#610c27]/20 focus:border-[#610c27]/40 transition-all backdrop-blur-sm" required />

                <div className="relative">
                  <input id="login-password" type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Passcode" className="w-full bg-[#f9f8f6]/50 border border-[#e0ddd8] rounded-xl px-5 pr-16 py-3.5 text-[#1a1a1a] placeholder-[#999] text-sm focus:outline-none focus:ring-2 focus:ring-[#610c27]/20 focus:border-[#610c27]/40 transition-all backdrop-blur-sm" required />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#999] hover:text-[#610c27] transition-colors text-xs font-bold uppercase tracking-wider flex items-center gap-1.5">
                    {showPassword ? <><EyeOff className="w-3.5 h-3.5" /> Hide</> : <><Eye className="w-3.5 h-3.5" /> Show</>}
                  </button>
                </div>

                <button type="button" className="text-xs font-medium text-[#666] hover:text-[#610c27] transition-colors">Having trouble in sign in?</button>
              ...
              <div className="flex items-center gap-4 my-6">
                ...
                <span className="text-[11px] uppercase tracking-[0.12em] font-semibold text-[#999]">Or sign in with</span>
                <div className="flex-1 h-px bg-[#e0ddd8]" />
              </div>
              ...
              <p className="text-center mt-6 text-sm text-[#666] font-medium">
                Don't have an account?{' '}
                <Link to="/register" className="text-[#610c27] font-bold hover:underline underline-offset-4">Register Now</Link>
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4 my-auto py-8">
            {features.map((f, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.4 + i * 0.12 }}
                ...
                <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center text-[#e3c1b4]">{f.icon}</div>
                <div>
                  <h3 className="text-sm font-bold text-white uppercase tracking-wide mb-0.5">{f.title}</h3>
